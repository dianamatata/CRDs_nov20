...
for data_type in  'methyl' 'hist' ; do
        for cell_type in 'neut' 'mono' 'tcell' ; do
                for module in 'mean' 'loom' ; do
                	...
                	for k in $(seq 1 $K); do
                        	OUT1=$OUTDIR/nominal/${data_type}_${cell_type}_${module}_CRD_gene_chunk$k
                         	cmd1="QTLtools cis --vcf $MOD --bed $RNA --nominal --chunk $k $K --out ${OUT1}.txt"
                              	eval $cmd1
			done
                done
        done
	...
	for condition in 'mapping_CRD_gene' ; do
		for data_type in  'methyl' 'hist' ; do
			for cell_type in 'neut' 'mono' 'tcell' ; do
				...
				cat $OUTDIR/nominal/${name}_*.txt | gzip -c > $OUTDIR/nominal_merged/${name}_${condition}_ALL.txt.gz
			done
		done
	done
done
