...
for data_type in  'methyl' 'hist' ; do
        for cell_type in 'neut' 'mono' 'tcell' ; do
			...
			for k in $(seq 1 $K); do
                		OUT1=$OUTDIR/permuts/${data_type}_${cell_type}_${module}_CRD_QTL_$k
				cmd="QTLtools cis --vcf $VCF --bed $BED --permute 1000  --chunk $k $K --out ${OUT1}.txt"
				eval $cmd
			done
		done
	done
...
for data_type in  'methyl' 'hist' ; do
        for cell_type in 'neut' 'mono' 'tcell' ; do
			...
			name=${data_type}_${cell_type}_${module}
                        cat $OUTDIR/permuts/${name}_*.txt | gzip -c > $OUTDIR/merged/${name}_${condition}_ALL.txt.gz
                done
        done
done
