...
# file='/Users/dianaavalos/Programming/A_CRD_plots/8_PEAKS/methyl_tcell.corr.chr8.txt'
# a=fread(file)
...
for(data_type in data_types){
  for(cell_type in cell_types){
      ...
      if(reImport){
        ...
        # file=as.data.frame(data.table::fread(filename, head=FALSE, stringsAsFactors=FALSE))
        ...
        save(cordata,file=paste0(out_dir,'/',name_condition,"_cordata.RData"))
      } else {
        cat("Importing correlation data...\n")
        load(file=paste0(out_dir,'/',name_condition,"_cordata.RData"))
      }
