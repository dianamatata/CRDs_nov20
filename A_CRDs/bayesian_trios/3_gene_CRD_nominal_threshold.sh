...
mkdir -p $OUTDIR $OUTDIR/mapping_CRD_gene_nominal_$permuts $OUTDIR/merged_nominal_$permuts
...
for data_type in  'methyl' 'hist' ; do
        for cell_type in 'neut' 'mono' 'tcell' ; do
			...
			TH=$OUTDIR/significants_$permuts/FDR_0.01_${data_type}_${cell_type}_${module}_mapping_CRD_gene_ALL.thresholds.txt
                        for k in $(seq 1 $K); do
                                OUT1=$OUTDIR/mapping_CRD_gene_nominal_$permuts/${data_type}_${cell_type}_${module}_CRD_gene_chunk$k
        ...
        for condition in 'mapping_CRD_gene' ; do
                for data_type in  'methyl' 'hist' ; do
                        for cell_type in 'neut' 'mono' 'tcell' ; do
                                ...
                                cat $OUTDIR/mapping_CRD_gene_nominal_$permuts/${name}_*.txt | gzip -c > $OUTDIR/merged_nominal_$permuts/${name}_${condition}_ALL.txt.gz
                        done
                done
        done
done
